...
class JacobsonTimeoutCalculator:
  ...
  def __update_statistics(self, server_received_at, sequence_number_received): 
    ...
    print(f"[TIMEOUT] - Expected to receive message of sequence number {sequence_number_received} at least until {local_time_timeout_at.strftime('%Y-%m-%d %H:%M:%S %Z')}, but it was received at {local_time_server_received_at.strftime('%Y-%m-%d %H:%M:%S %Z')}")
...
for i in range(0, 18): 
  with open(f"./traces_ufpr_ufsm_week_day/raw/log_{i}.txt") as file:
    for x, line in enumerate(file, start=1):
      ...
      calculated_timeout = jac_timeout_calculator.calculate_timeout_at(server_received_at, sequence_number_received)